...
for (const file of servapps) {
  ...
  for (const screenshot of screenshots) {
    servapp.screenshots.push(`https://Veneziaisking.github.io/cosmos-servapps/${file}/screenshots/${screenshot}`)
  }

  if(fs.existsSync(`./servapps/${file}/artefacts`)) {
    const artefacts = fs.readdirSync(`./servapps/${file}/artefacts`)
    for(const artefact of artefacts) {
      servapp.artefacts[artefact] = (`https://Veneziaisking.github.io/cosmos-servapps/${file}/artefacts/${artefact}`)
    }
  }

  servapp.icon = `https://Veneziaisking.github.io/cosmos-servapps/${file}/icon.png`
  //Common Format,used by most
  const YMLComposeSource =  `https://Veneziaisking.github.io/cosmos-servapps/${file}/docker-compose.yml`;
  if(fs.existsSync(`./servapps/${file}/docker-compose.yml`)) {
    servapp.compose = YMLComposeSource;
  }
  //Cosmos Legacy Format
  const CosmosComposeSource =  `https://Veneziaisking.github.io/cosmos-servapps/${file}/cosmos-compose.json`; 
