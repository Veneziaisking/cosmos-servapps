...
for (const file of servapps) {
  ...
  for (const screenshot of screenshots) {
    servapp.screenshots.push(`https://veneziaisking.github.io/cosmos-servapps/servapps/${file}/screenshots/${screenshot}`)
  }

  if(fs.existsSync(`./servapps/${file}/artefacts`)) {
    const artefacts = fs.readdirSync(`./servapps/${file}/artefacts`)
    for(const artefact of artefacts) {
      servapp.artefacts[artefact] = (`https://veneziaisking.github.io/cosmos-servapps/servapps/${file}/artefacts/${artefact}`)
    }
  }

  servapp.icon = `https://veneziaisking.github.io/cosmos-servapps/servapps/${file}/icon.png`
  //Common Format,used by most
  const YMLComposeSource =  `https://veneziaisking.github.io/cosmos-servapps/servapps/${file}/docker-compose.yml`;
  if(fs.existsSync(`./servapps/${file}/docker-compose.yml`)) {
    servapp.compose = YMLComposeSource;
  }
  //Cosmos Legacy Format
  const CosmosComposeSource =  `https://veneziaisking.github.io/cosmos-servapps/servapps/${file}/cosmos-compose.json`; 
...
for (const servapp of servappsJSON) {
  ...
  for (let i = 0; i < servapp.screenshots.length; i++) {
    servapp.screenshots[i] = servapp.screenshots[i].replace('https://veneziaisking.github.io/cosmos-servapps', 'http://localhost:3000')
  }
  for (const artefact in servapp.artefacts) {
    servapp.artefacts[artefact] = servapp.artefacts[artefact].replace('https://veneziaisking.github.io/cosmos-servapps', 'http://localhost:3000')
  }
}
